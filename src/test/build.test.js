...
describe("빌드 테스트", () => {
  it("ES 모듈 빌드 파일이 존재해야 합니다", () => {
    const esPath = resolve(process.cwd(), "dist/sweetpd.es.js");
    expect(existsSync(esPath)).toBe(true);
  });

  it("CommonJS 빌드 파일이 존재해야 합니다", () => {
    const cjsPath = resolve(process.cwd(), "dist/sweetpd.cjs.js");
  ...
  it("package.json의 main과 module 필드가 올바르게 설정되어야 합니다", async () => {
    ...
    expect(pkg.default.main).toBe("./dist/sweetpd.cjs.js");
    expect(pkg.default.module).toBe("./dist/sweetpd.es.js");
  });
  ...
  it("package.json의 exports 필드가 올바르게 설정되어야 합니다", async () => {
    ...
    expect(pkg.default.exports["."]).toEqual({
      import: "./dist/sweetpd.es.js",
      require: "./dist/sweetpd.cjs.js",
    });
  });
});
